...
class CartPoleCustomEnv(CartPoleModEnv):
    def __init__(self, config, true_value):
        ...
        self.nuisance_name = config["nuisance"]["name"]
        self.bounds = config["nuisance"]["bounds"]
        low = np.append(low, self.bounds[0])
        high = np.append(high, self.bounds[1])
    ...
    def step(self, action):
        ...
        return np.append(state, self.nuisance_obs), reward, done, info

    def reset(self, nuisance=None):
        state = super().reset()
        ...
        return np.append(state, self.nuisance_obs)
